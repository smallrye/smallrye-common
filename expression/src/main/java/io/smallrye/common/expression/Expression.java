...
public final class Expression {
            ...
            if (key.startsWith("env.")) {
                ...
                if (val == null) {
                    if (failOnNoDefault && !c.hasDefault()) {
                        throw Messages.msg.unresolvedEnvironmentProperty(env);
                    }
                    c.expandDefault();
                } else {
                    b.append(val);
                ...
                if (val == null) {
                    if (failOnNoDefault && !c.hasDefault()) {
                        throw Messages.msg.unresolvedSystemProperty(key);
                    }
                    c.expandDefault();
                } else {
                    b.append(val);
    ...
    public String evaluateWithEnvironment(boolean failOnNoDefault) {
        return evaluate((c, b) -> {
            ...
            if (val == null) {
                if (failOnNoDefault && !c.hasDefault()) {
                    throw Messages.msg.unresolvedEnvironmentProperty(key);
                }
                c.expandDefault();
            } else {
                b.append(val);
    ...
    private static IllegalArgumentException invalidExpressionSyntax(final String string, final int index) {
        String msg = Messages.msg.invalidExpressionSyntax(index);
